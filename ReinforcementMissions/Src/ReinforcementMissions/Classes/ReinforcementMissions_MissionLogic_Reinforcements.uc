class ReinforcementMissions_MissionLogic_Reinforcements extends XComGameState_MissionLogic config(ReinforcementMissions);
...
function EventListenerReturn CheckRedAlert(Object EventData, Object EventSource, XComGameState GameState, Name EventID)
{
	...
	local ReinforcementMissions_MissionLogic_Reinforcements NewMissionState;
	...
	if (!ProgressionActive && EventAbilityIs("RedAlert", EventData, GameState))
	{
		...
		NewMissionState = ReinforcementMissions_MissionLogic_Reinforcements(NewGameState.CreateStateObject(class'ReinforcementMissions_MissionLogic_Reinforcements', ObjectID));
